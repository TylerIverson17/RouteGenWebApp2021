...
public class GuidenceSystem {
	...
	File file2 = new File("dialogueOutput.txt");
...
//Mark ->
	public void createDirections(Queue<Integer> routes) {
		...
		while (!routes.isEmpty()) {
			...
			// adds the house addresses to the output arraylist on every iteration
			output.add(realhousenumber + "");
			output.add(currentstreet + "");
			output.add(nextrealhousenumber + "");
			output.add(nextstreet + "");
...
//MARK ->	
			if (streetdiff == 0) { // THE TWO HOUSES ARE ON THE SAME STREET
				...
				if (currenthouse - nexthouse < 0) {
					...
					dialogue.add("travel SOUTH past " + housediff + " houses");
					
					output.add("SOUTH " + housediff);
					output.add("Null");
					output.add("Null");
				...
				if (currenthouse - nexthouse > 0) {
					...
					dialogue.add("travel NORTH past " + housediff + " houses");
					
					output.add("NORTH " + housediff);
					output.add("Null");
					output.add("Null");
					houseOddOrEven(true);
				}
...
//MARK ->	// case where houses are NOT on the same street //
				...
				if (currentstreet - nextstreet < 0) {// HEAD EAST TO TRAVEL TO LOCATION WITH STREET THAT IS LARGER THAN
														// THE STARTING
					travelWest = false;
...
//MARK ->	//North -> east/west -> North
		...
		writeFile(file2, dialogue);
	}

//MARK - > // WRITES THE ARRAYLIST OF OUTPUT STRINGS TO OUTPUT.TXT	
	public void writeFile(File somefile, ArrayList<String> textTowrite) {
		temp = "";
		try {
			BufferedWriter out = new BufferedWriter(new FileWriter(somefile));
			for (int i = 0; i < textTowrite.size(); i++) {
				temp = textTowrite.get(i);
...
//MARK -> DIALOGUE FOR DIRECTIONS 
	public void routeDeclaration() {
		
		System.out.print("\nRoute starts at: " + realhousenumber + " " + currentstreet);
		temp = "Route starts at: " + realhousenumber + " " + currentstreet;
		...
		if (currentstreet % 10 > 3 || currentstreet % 10 == 0) {
		...
		temp = "Travel to: " + nextrealhousenumber + " " + nextstreet;
		if (nextstreet % 10 == 1) {
			System.out.print("st street\n");
			temp += "st street";
		}
		...
		if (nextstreet % 10 > 3 || nextstreet % 10 == 0) {
			System.out.print("th street\n");
			temp += "th street";
		}
...
//MARK ->
	/*
	 * THE DIALOGUES BELOW NEEED 3 THINGS TO RUN distance: is the units between the
	 * house your at to the next avenue
	 * 
	 * streetdiff: to print change in streets
	 * 
	 * distance2: gives you closest distance from the house your at to the next
	 * avenue ****if the distance between currenthouse/10 and nexthouse /10 is
	 * greater than 2 i.e. the houses are 2 blocks away ****then multiply the
	 * difference in blocks by 10
	 *
	 * 
	 * 
	 */
	public void northNorth() {
		...
		if (travelWest) {
			output.add("NORTH " + distance);
			output.add("LEFT " + streetdiff);
			output.add("NORTH " + distance2);
			...
			dialogue.add("Travel NORTH past " + distance + " house(s)");
			dialogue.add("Then go LEFT " + streetdiff + " street(s)");
			dialogue.add("Then travel NORTH past " + distance2 + " house(s)");
		...
		// IF TRAVELWEST IS FALSE -- TRAVELING EAST
			...
			output.add("NORTH " + distance);
			output.add("RIGHT " + streetdiff);
			output.add("NORTH " + distance2);
			System.out.println("Travel NORTH past " + distance + " house(s)");
			System.out.println("Then go RIGHT " + streetdiff + " street(s)");
			System.out.println("Then travel NORTH past " + distance2 + " houses(s)");
			dialogue.add("Travel NORTH past " + distance + " house(s)");
			dialogue.add("Then go RIGHT " + streetdiff + " street(s)");
			dialogue.add("Then travel NORTH past " + distance2 + " houses(s)");
			houseOddOrEven(true);
...
//MARK ->
	public void southSouth() {
		...
		if (travelWest) {
			output.add("SOUTH " + distance);
			output.add("LEFT " + streetdiff);
			output.add("SOUTH " + distance2);
			System.out.println("Travel SOUTH past " + distance + " house(s)");
			System.out.println("Then go LEFT " + streetdiff + " street(s)");
			System.out.println("Then travel SOUTH past " + distance2 + " house(s)");
			dialogue.add("Travel SOUTH past " + distance + " house(s)");
			dialogue.add("Then go LEFT " + streetdiff + " street(s)");
			dialogue.add("Then travel SOUTH past " + distance2 + " house(s)");
			houseOddOrEven(false);
		}
		// IF TRAVELWEST IS FALSE -- TRAVELING EAST
		else {
			...
			dialogue.add("Travel SOUTH past " + distance + " house(s)");
			dialogue.add("Then go RIGHT " + streetdiff + " street(s)");
			dialogue.add("Then travel SOUTH past " + distance2 + " houses(s)");
			houseOddOrEven(false);
